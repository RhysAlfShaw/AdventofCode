...
fn blick(data: Vec<i64>) -> Vec<i64> {
    ...
    for &num in &data {
        if num == 0 {
            ...
            new_data.push(first_half.parse().unwrap());
            new_data.push(second_half.parse().unwrap());
        } else {
            new_data.push(num * 2024);
        }
    }
...
fn main() {
    ...
    for _ in 0..num_blicks {
        test_data_blicked = blick(test_data_blicked);
    ...
    for i in 0..real_data_blicked.len() {
        ...
        let handle = thread::spawn(move || {
            ...
            for j in 0..num_blicks {
                println!("Thread: {}. Blick Number: {}",i, j);
    ...
    //     real_data_blicked = blick(real_data_blicked);
